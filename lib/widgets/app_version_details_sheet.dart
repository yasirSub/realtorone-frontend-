...
/// Tap-to-open version details (profile footer, settings, etc.).
class AppVersionDetailsSheet {
  AppVersionDetailsSheet._();
  ...
  static Future<void> show(BuildContext context) {
    return showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => const _AppVersionSheetBody(),
    );
  }
}

class _AppVersionSheetBody extends StatefulWidget {
  const _AppVersionSheetBody();

  @override
  State<_AppVersionSheetBody> createState() => _AppVersionSheetBodyState();
}

class _AppVersionSheetBodyState extends State<_AppVersionSheetBody> {
  ...
  Future<void> _refresh() async {
    ...
    try {
      final info = await AppVersionService.load(bustCache: true);
      if (mounted) {
        setState(() {
          _info = info;
          _loading = false;
  ...
  Widget _statusCard(AppVersionInfo info, bool isDark) {
    ...
    final color =
        needsUpdate ? const Color(0xFFF97316) : const Color(0xFF10B981);
...
/// Small tappable version label for footers.
class AppVersionTapLabel extends StatelessWidget {
  const AppVersionTapLabel({
    super.key,
    required this.label,
    this.style,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: InkWell(
        onTap: () => AppVersionDetailsSheet.show(context),
        ...
        child: Padding(
          ...
          child: Text(
            ...
            style: (style ?? defaultStyle).copyWith(
              ...
              decorationColor: (style?.color ?? defaultStyle.color)?.withValues(alpha: 0.5),
            ),
          ),
        ),
      ),
